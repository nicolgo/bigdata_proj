...
class Net(nn.Module):
    def __init__(self, n_inputs, n_outputs):
        ...
        self.hidden1 = nn.Linear(n_inputs, n_inputs*3)
        kaiming_uniform_(self.hidden1.weight, nonlinearity='relu')
        self.dropout1 = nn.Dropout(0.3)
        self.act1 = nn.ReLU()
        # second hidden layer
        self.hidden2 = nn.Linear(n_inputs*3, n_outputs*3)
        kaiming_uniform_(self.hidden2.weight, nonlinearity='relu')
        self.dropout2 = nn.Dropout(0.3)
        self.act2 = nn.ReLU()
        # third hidden layer and output
        self.hidden3 = nn.Linear(n_outputs*3, n_outputs)
...
if __name__ == '__main__':
    ...
    #     train_dataset, train_dl_batch_size, need_unify=False, is_test=False)
    train_tensor = TensorDataset(torch.tensor(X), torch.tensor(y))
    train_dl = DataLoader(
        train_tensor, batch_size=train_dl_batch_size, shuffle=True)
    ...
    evaluate_model(class_model, test_dl, test_dl_batch_size)
